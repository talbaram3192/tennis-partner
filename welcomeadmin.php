...
if(isset($_GET['bestp'])){
         ...
         include('connect.php');
            $checkvote = $conn->query("select voted from user where name = '$_SESSION[name]' ");
            while($check = $checkvote->fetch_assoc()){
              $didvote = $check['voted'];
            }
            if($didvote == 0){
            $conn->query("update user set voted = 1 where name = '$_SESSION[name]' ");
            ...
            $result=$conn->query("select * from weeklyquestions where choice = 'roger' ");
          
            while($row = $result->fetch_assoc()) {
              $roger = $row['numchoice'];
            }
            $result=$conn->query("select * from weeklyquestions where choice = 'nole' ");
          
            while($row = $result->fetch_assoc()) {
              $nole = $row['numchoice'];
            }
            $result=$conn->query("select * from weeklyquestions where choice = 'nadal' ");
          
            while($row = $result->fetch_assoc()) {
              $nadal = $row['numchoice'];
            }
            $result=$conn->query("select * from weeklyquestions where choice = 'diego' ");
          
            while($row = $result->fetch_assoc()) {
              $diego = $row['numchoice'];
            }
            $result=$conn->query("select * from weeklyquestions where choice = 'isner' ");
          
            while($row = $result->fetch_assoc()) {
              $isner = $row['numchoice'];
            }
            $result=$conn->query("select * from weeklyquestions where choice = 'cilic' ");
          
            while($row = $result->fetch_assoc()) {
              $cilic = $row['numchoice'];
            }
            $result=$conn->query("select * from weeklyquestions where choice = 'stef' ");
          
            while($row = $result->fetch_assoc()) {
              $stef = $row['numchoice'];
            }
            $result=$conn->query("select * from weeklyquestions where choice = 'medvedev' ");
      ...
      function myfunc2(){
        ...
        $x = $_SESSION["name"];
        $conn->query("update games set player2 = '$x' where serial = $serial"); 
      ...
      function myfunc3(){
         include('connect.php');
          ...
          $conn->query("delete from games where serial = '$delgame'");
...
<aside class="x">                                <!-- side a -->
	<div class="y">
		...
		<form action="welcomeadmin.php" method="GET">
		...
		while($row4 = $result5->fetch_assoc()){ ?>
           ...
           <div class="ga">
           	  <?php if($row4["player2"]=="none"){
           	  	...
           	  	echo $row4["player1"]; ?> <b>vs <?php echo "<form action=welcomeadmin.php method=post><input type=submit name=gameOn value=Play_" . $row4["serial"]. "> </form>" ?>
           	  	<?php }else{ 
           	  		echo $row4["player1"]; ?> <b>vs <?php echo $row4["player2"]; } ?>

		      <p> <?php echo $row4["gtime"]; ?></p>
		      <p><?php echo $row4["gdate"]; ?></p>
		      <?php echo "<div class=delgame><form action=welcomeadmin.php method=post><input type=submit value=delete_game_num_".$row4["serial"].  " name=delgame></form> </div>" ; ?>  
...
<aside class="x">                                     <!-- side b -->
	...
	<table><tr><th><b>players name</th><th><b>players level</th><th><b>delete player</th>";
	       while($row = $result->fetch_assoc()) {
             echo "<tr><td>" . $row["name"]. "</td><td>" . $row["level"]. "</td><td><form action=welcomeadmin.php method=post><input type=submit name=delete value=".$row["id"]."></form></td></tr>";
          }
     echo "</table>";
} else {
     echo "0 results";
...
<form method="GET" action="welcomeadmin.php">
